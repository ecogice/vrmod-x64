...
hook.Add ("VRMod_Input", "VRModButtonPresser", function (action, state)     -- hook that fires whenever any VR input is made
...
else if action == "boolean_right_pickup" then       -- same thing as above but for the right hand
    net.Start("VRButtonPresserMessage")
...
net.Receive("VRButtonPresserMessage", function (len, ply)           -- receives the net message configured above, and it includes information about the controller input
...
hook.Add ("PlayerTick", "VRButtonTicker", function(ply)      -- this hook is needed to continuously fire the 'use' input later on - 
    ...
    if !ply:Alive() then            -- checks if the player is alive, so we can just ignore all the code below if the player is dead - so you can't activate buttons while dead
...
hook.Add ("PlayerUse", "NoTelepathicButtonPressing", function (ply, ent)        -- this is here to stop VR players with this addon from activating buttons through the 'regular' use function - meaning they could activate buttons from a distance, without touching them
    if ent:GetClass() == "func_button" and vrmod.IsPlayerInVR(ply) or ent:GetClass() == "func_rot_button" and vrmod.IsPlayerInVR(ply) or ent:GetClass() == "item_healthcharger" and vrmod.IsPlayerInVR(ply) or ent:GetClass() == "item_suitcharger" and vrmod.IsPlayerInVR(ply) or ent:GetClass() == "item_ammo_crate" and vrmod.IsPlayerInVR(ply) then return false
    end
end)
